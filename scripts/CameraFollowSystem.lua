...
function CLASS:SetActiveCameraSettings(cameraSettings)
	assert(cameraSettings ~= nil, "Camera System Argument Error: Argument 1 nil or missing")
	assert(typeof(cameraSettings) == "string", "Camera System Argument Error: string expected, got " .. typeof(cameraSettings))
	assert(self.CameraSettings[cameraSettings] ~= nil, "Camera System Argument Error: Attempt to set unrecognized camera settings " .. cameraSettings)
	if (self.IsEnabled == false) then
		warn("Camera System Logic Warning: Attempt to change active camera settings without enabling camera system")
		return
	end
...
function CLASS:SetCharacterAlignment(aligned)
	assert(aligned ~= nil, "Camera System Argument Error: Argument 1 nil or missing")
	assert(typeof(aligned) == "boolean", "Camera System Argument Error: boolean expected, got " .. typeof(aligned))
	if (self.IsEnabled == false) then
		warn("Camera System Logic Warning: Attempt to change character alignment without enabling camera system")
		return
	end
...
function CLASS:SetMouseStep(steppedIn)
	assert(steppedIn ~= nil, "Camera System Argument Error: Argument 1 nil or missing")
	assert(typeof(steppedIn) == "boolean", "Camera System Argument Error: boolean expected, got " .. typeof(steppedIn))
	if (self.IsEnabled == false) then
		warn("Camera System Logic Warning: Attempt to change mouse step without enabling camera system")
		return
	end
...
function CLASS:SetShoulderDirection(shoulderDirection)
	assert(shoulderDirection ~= nil, "Camera System Argument Error: Argument 1 nil or missing")
	assert(typeof(shoulderDirection) == "number", "Camera System Argument Error: number expected, got " .. typeof(shoulderDirection))
	assert(math.abs(shoulderDirection) == 1, "Camera System Argument Error: Attempt to set unrecognized shoulder direction " .. shoulderDirection)
	if (self.IsEnabled == false) then
		warn("Camera System Logic Warning: Attempt to change shoulder direction without enabling camera system")
		return
	end
...
function CLASS:Enable()
	assert(self.IsEnabled == false, "Camera System Logic Error: Attempt to enable without disabling")
...
function CLASS:Disable()
	assert(self.IsEnabled == true, "Camera System Logic Error: Attempt to disable without enabling")
...
USER_INPUT_SERVICE.InputChanged:Connect(function(input)
  if input.UserInputType == Enum.UserInputType.MouseWheel then
    if input.Position.Z > 0 then
				singleton.dataTable.CameraSettings.DefaultShoulder.FieldOfView -= 5
		else
				singleton.dataTable.CameraSettings.DefaultShoulder.FieldOfView += 5
		end
